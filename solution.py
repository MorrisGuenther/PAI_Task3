...
class BO_algo(object):
    def __init__(self):
        ...
        kernel_constraint = ConstantKernel(constant_value=3.5, constant_value_bounds = "fixed") * RBF(length_scale = 2, length_scale_bounds = "fixed")
        self.constraint_model = GaussianProcessRegressor(kernel = kernel_constraint)

        # TODO : GP model for your objective function
        kernel_objective = ConstantKernel(constant_value=1.5, constant_value_bounds = "fixed") * RBF(length_scale = 1.5, length_scale_bounds = "fixed")
        self.objective_model = GaussianProcessRegressor(kernel = kernel_objective)
    ...
    def next_recommendation(self) -> np.ndarray:
        ...
        if self.previous_points == []:
            next_recommendation = np.array([[3,3]])
        else:
    ...
    def optimize_acquisition_function(self) -> np.ndarray:  # DON'T MODIFY THIS FUNCTION
        ...
        for _ in range(20):
            ...
            result = fmin_l_bfgs_b(objective, x0=np.array([x0, x1]), bounds=domain_x,approx_grad=True)
    ...
    def acquisition_function(self, x: np.ndarray) -> np.ndarray:
        ...
        objective_posterior_mean = self.objective_model.predict(x.reshape(1,-1),return_std = True)[0]
        objective_posterior_std = self.objective_model.predict(x.reshape(1,-1),return_std = True)[1]

        previous_points_ = np.concatenate(self.previous_points, axis = 0)
        previous_points = np.reshape(previous_points_, newshape = (-1,4))
        target = self.objective_model.predict(previous_points[:,0:2]).min() # target = minimum of the expected value of the objective
        
        z = (target - objective_posterior_mean) / objective_posterior_std

        cdf_z = norm.cdf(z, loc=0, scale=1)
        pdf_z = norm.pdf(z, loc=0, scale=1)

        expected_improvement = objective_posterior_std * (z * cdf_z + pdf_z)
        
        # Computing the probability that the constraint is satisfied
        constraint_posterior_mean = self.constraint_model.predict(x.reshape(1,-1),return_std = True)[0]
        constraint_posterior_std = self.constraint_model.predict(x.reshape(1,-1),return_std = True)[1]

        constraint_prob = norm.cdf(0, loc=constraint_posterior_mean, scale=constraint_posterior_std)

        # value of the acquisition function
        af_value = expected_improvement * constraint_prob

        return af_value.ravel()
    ...
    def add_data_point(self, x: np.ndarray, z: float, c: float):
        ...
        previous_points_ = np.concatenate(self.previous_points, axis = 0)
        previous_points = np.reshape(previous_points_, newshape = (-1,4))
        self.constraint_model.fit(previous_points[:,0:2], previous_points[:,3:])
        self.objective_model.fit(previous_points[:,0:2], previous_points[:,2:3])
    ...
    def get_solution(self) -> np.ndarray:
        ...
        previous_points_ = np.concatenate(self.previous_points, axis = 0)
        previous_points = np.reshape(previous_points_, newshape = (-1,4))      
        points_satisfying_constraint = previous_points[previous_points[:,3]<= 0,:]
        if points_satisfying_constraint.shape[0] == 0:
            points_satisfying_constraint = previous_points
        ind = np.argmin(points_satisfying_constraint[:,2])
        x_opt = points_satisfying_constraint[ind,0:2]
        return x_opt
